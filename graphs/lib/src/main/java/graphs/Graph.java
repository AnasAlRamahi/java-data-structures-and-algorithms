...
//        Node<T> newNode = new Node<T>(value);
    ...
    public String edgeListString(Node n){
        String result = "[" + n.value + " -> ";
        for (int i = 1 ; i < getNeighbors(n).size() - 1 ; i++){
            result += getNeighbors(n).get(i).value + ", ";
        }
        result += getNeighbors(n).get(getNeighbors(n).size()-1).value + "]";
        return result;
    }
